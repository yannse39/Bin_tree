...
#Définie nos fichiers de sortie attendue
        EXPECTED_OUTPUT_FILE = ("tests.out"
                                "tests_2.out"
                                "tests_3.out"
                                "tests_4.out"
                                "tests_5.out")
...
#Voici notre fonction pour compiler notre fichier C
            function
            compile_test(){ local file = "$1" local output_file = "$2"

                                gcc
                                - o "$output_file"
                                    "$file"
                                - L"$BIN_DIR" - lmybintree - Wall - Werror }
...
function run_test()
{
  ...
  if [ $? -eq 0 ];
  then echo "Test $test_file réussi" else echo
      "Test $test_file échoué" echo "Sortie attendue:" cat
      "$expected_output_file" echo "Sortie obtenue:" cat "$output_file" fi
...
for i in "${!TEST_FILE[@]}";
do
run_test "${BIN_DIR}/${TEST_FILE[$i]}"
         "${EXPECTED_OUTPUT_FILE[$i]}" done
